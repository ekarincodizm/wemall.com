...
(function($) {
    ...
    $.fn.SubCategoryMenu = function(options) {
        ...
        var defaults = {
            ...
            subCategoryMenuView: '.sub-categories-view'
        };
        ...
        return $('.category-menu > li').each(function(idx) {
            $(this).hover(function() {
                ...
                if (prevIdx !== undefined && prevIdx !== idx) {
                    ...
                    $('.category-menu > li:eq(' + prevIdx + ')').find(settings.subCategoryMenuView).css({display: 'none'});
                }
                $('.category-menu > li:eq(' + prevIdx + ')').find(settings.subCategoryMenu).removeAttr('style');
                $('.category-menu > li:eq(' + prevIdx + ')').find(settings.subCategoryMenuView).removeAttr('style');
                $(this).find(settings.subCategoryMenu).css({display: 'block'});
                $(this).find(settings.subCategoryMenuView).css({display: 'block'});
                prevIdx = idx;
                clearTimeout(timeout);
            }, function() {
                // console.log('out');
                timeout = setTimeout(function() {
                    $(settings.subCategoryMenu).css({display: 'none'});
                    $(settings.subCategoryMenuView).css({display: 'none'});
                }, '500');
            });
        });
    };
...
function LevelBFunctions() {
    ...
    $.fn.BrandStory = function() {
        return this.each(function() {
            $(this).hover(function() {
                $(this).find('.prev_lvb').css({display: 'block'});
                $(this).find('.next_lvb').css({display: 'block', zIndex: 100});
            }, function() {
                $(this).find('.prev_lvb').css({display: 'none'});
                $(this).find('.next_lvb').css({display: 'none'});
            });
        });
    };

    $('.brand_banner').BrandStory();
	
        var touch_pos = null
	$('.promotionlist_lvb').on('touchstart', function(e){
//            var originTouch = e.originalEvent;
//            $.each(originTouch, function(idx, event){
...
//            return false;
            ...
            var _url = $('a:eq(0)', $(this)).attr('href');
            location.href = _url;
        });
}
...
function ProfileFunctions() {
    ...
    $('.jq_watermark').on('keypress copy paste', function(e) {
...
function globalAlert(title, msg) {

    if ( ! msg) return;

    $('#global-alert .alert-title').text(__(title));
    $('#global-alert .alert-message').text(__(msg));
