...
MiniCart.Main = (function($){
    ...
    <% if(MiniCart.Main.config.showTrueyou == true && MiniCart.Main.config.isTUAvailablePage == true){ %>  \n\
        <% if( (MiniCart.Main.config.usertrueyou == false && MiniCart.Main.config.isLogedin == true ) || ( checkout.cart_trueyou == undefined && MiniCart.Main.config.isLogedin == false ) ) { %>\n\
            <div class='total-list trueyou-container'>  \n\
                <div class='box-discount'>  \n\
                    <p class='control-label-total clr-5 text-blink'><%= __('cart-discount-trueyou') %></p>  \n\
                    <div class='sum edit-box'><a href='#' id='auth-trueyou' class='clr-6'><%= __('cart-verify-trueyou-btn') %></a></div>    \n\
                    <div class='clear'></div>   \n\
                </div>  \n\
            </div>  \n\
        <% } %> \n\
        <div class='total-list trueyou-container' style='display: <% if( (MiniCart.Main.config.usertrueyou == false && MiniCart.Main.config.isLogedin == true ) || ( checkout.cart_trueyou == undefined && MiniCart.Main.config.isLogedin == false ) ){ print('none'); }else{ print('block'); } %>;'>   \n\
            <div class='box-discount'>  \n\
                <p class='control-label-total clr-7 text-ani-1'>   \n\
                    <span><%= __('cart-discount-trueyou') %></span> \n\
                    <% if( MiniCart.Main.config.usertrueyou_card != undefined || checkout.cart_trueyou != undefined ){ %> \n\
                        <span>  \n\
                        <% if( MiniCart.Main.config.usertrueyou_card != undefined && MiniCart.Main.config.isLogedin == true ) { %>\n\
                            <% if( MiniCart.Main.config.usertrueyou_card  == 'red' ){  %>    \n\
                                <img width='30' src='<%= site_url + 'themes/checkout/assets/images/card-red.png' %>' />   \n\
                            <% } else if(MiniCart.Main.config.usertrueyou_card == 'black' ) { %>    \n\
                                <img width='30' src='<%= site_url + 'themes/checkout/assets/images/card-black.png' %>' /> \n\
                            <% } %> \n\
                        <% } else if( checkout.cart_trueyou != undefined && MiniCart.Main.config.isLogedin == false ) { %>\n\
                            <% if( checkout.cart_trueyou.card == 'red' ){  %>    \n\
                                <img width='30' src='<%= site_url + 'themes/checkout/assets/images/card-red.png' %>' />   \n\
                            <% } else if( checkout.cart_trueyou.card == 'black') { %>    \n\
                                <img width='30' src='<%= site_url + 'themes/checkout/assets/images/card-black.png' %>' /> \n\
                            <% } %> \n\
                        <% } %>\n\
                        </span> \n\
                ...
                <div class='sum clr-7 text-ani-1'><%= (MiniCart.Main.config.trueyouDiscount>0 ? '-' : '') +  MiniCart.Main.config.trueyouDiscount.formatMoney(2) %></div>   \n\
            </div>  \n\
        </div>  \n\
        <div class='clear'></div>   \n\
    <% } %> \n\
