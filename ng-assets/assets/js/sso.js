...
var centralLogout = function(data) {
    ...
    $.getJSON("https://loginservice.truelife.com/checkCentralizeCookies.php?jsoncallback=?", function (response) {
        if (response.jsonReturn.uid != undefined) {
            ...
            $.getJSON("https://loginservice.truelife.com/centralizeDeAuthen.php?jsoncallback=?", {app_id: data.app_id, secret: data.secret_key, access_token: access_token, type: data.type }, function (profile_data) {
                // redirect to referer or auth/login
                window.location.href = data.redirect;
            });
        }
